...
var url = require('url');
var sanitized = require('sanitize-html');
...
exports.create_process = function (request, response) {
    ...
    request.on('end', function () {
        ...
        db.query(`INSERT INTO author (name, profile) VALUES(?, ?)`, [post.name, post.profile], function (err, result) {
            if (err) {
                throw err;
            }
    ...
    db.query(`SELECT * FROM topic`, function (err, topics) {
        db.query(`SELECT * FROM author`, function (err, authors) {
            ...
            db.query(`SELECT * FROM author WHERE id=?`, [queryData.id], function (err, author) {
                ...
                <p>
                    <input type='text' name='name' placeholder='name' value=${sanitized(author[0].name)}>
                </p>
                <p>
                    <textarea name='profile' placeholder='description'>${sanitized(author[0].profile)}</textarea>
                </p>
                <p>
